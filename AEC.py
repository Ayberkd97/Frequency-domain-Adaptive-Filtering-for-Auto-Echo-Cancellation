...
# TODO: implement open-loop and closed-loop detectors based on coherence
# * recursively estimate auto PSDs of D, X, Y_hat
# * recursively estimate cross PSDs of (X,D) and (Y_hat,D)
# * compute magnitude-squared coherence (MSC) and average over frequency (optionally, weight by excitation power)
# * compare MSC to threshold, and halt adaptation if necessary
...
def calc_rho(x, d, L, gamma, c_11, c_12, c_22):
    ...
    #this is not in reference given by tutor, but in [2] in the report
...
def aec_double_talk(x, y, N, L, gamma1=0.9, gamma2=0.6,stepsize=0.2, threshold=0.6):
    ...
    for k_block in range(n_blocks):
        ...
        #mse[n_blocks] = np.sum(np.square(y_hat - y))/np.sum(np.square(y-np.mean(y)))
        ##implement y_hat-y/ total energy of the target
        #ERLE y^2/e^2
        
        #confirm the MSE and generate 3-4 plots. 
...
#try L = 1024, 512, 2048 plot 
# different gamma2 and stepsizes
